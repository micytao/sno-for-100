...
download_openshift_installer() {
    ...
    local ret=$(curl --write-out "%{http_code}" https://mirror.openshift.com/pub/openshift-v4/multi/clients/ocp/${OPENSHIFT_VERSION}/arm64/openshift-install-${system_os_flavor}.tar.gz -o ${RUN_DIR}/openshift-install-${system_os_flavor}.tar.gz)
    if [ "$ret" != "200" ]; then
...
download_openshift_cli() {
    ...
    #local ret=$(curl --write-out "%{http_code}" https://mirror.openshift.com/pub/openshift-v4/multi/clients/ocp/${OPENSHIFT_VERSION}/arm64/openshift-client-${system_os_flavor}-amd64.tar.gz -o ${RUN_DIR}/openshift-client-${system_os_flavor}-amd64.tar.gz)
    if [ "$ret" != "200" ]; then
